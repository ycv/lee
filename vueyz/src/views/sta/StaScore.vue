<template>
  <div>
    <div style="display: flex;justify-content: space-between">
      <div style="border: 0px solid #409eff; border-radius: 5px;
      box-sizing: border-box;padding: 5px;margin: 10px 0px;width: 80%;">
        <el-input placeholder="请输代码进行搜索，可以直接回车搜索..." prefix-icon="el-icon-search"
                  clearable
                  @clear="getStockRsiData"
                  style="width: 300px;margin-right: 10px" v-model="stockRsiParam.code"
                  @keydown.enter.native="getStockRsiData" :disabled="showStockRsiSearchView"></el-input>
        <el-button icon="el-icon-search" type="primary" @click="getStockRsiData" :disabled="showStockRsiSearchView">
          搜索
        </el-button>
...
export default {
  ...
  data() {
    return {
      showStockRsiSearchView: false,
      echartsOption: {
        // 全图默认背景
        backgroundColor: 'rgba(154,216,246,0.7)',
        // 默认色板
        color: ['rgb(255,234,0)', 'rgb(0,255,4)', 'rgb(0,102,255)', '#FFBC47'],
        // 图表标题
        title: {
      ...
      stockRsiParam: {
        code: '000593', beginDate: '', endDate: ''
      }
    }
  },
  created() {
    this.getStockRsiData();
  },
  mounted() {
  },
  methods: {
    getStockRsiData() {
      if (this.stockRsiParam.code == "") {
        mymessage.warning("请先填写搜索条件 ！");
      } else {
        this.$Progress.start();
        stockRsiApi.getStockRsiDataByParam(this.stockRsiParam).then(resp => {
          console.log(resp);
          if (resp && resp.status == 200) {
            this.echartsOption.title.subtext = this.stockRsiParam.code;
            this.echartsOption.title.text = resp.obj.beginYear + "--" + resp.obj.stockName;
            this.echartsOption.xAxis.data = resp.obj.dateList;
            for (let j = 0; j < this.echartsOption.series.length; j++) {
              let seriesName = this.echartsOption.series[j].name;
              if ("成交金额" == seriesName) {
                seriesName = "tradeAmount";
              }
              this.echartsOption.series[j].data = resp.obj[seriesName];
            }
            this.$nextTick(() => {
              this.myCharts();
            })
