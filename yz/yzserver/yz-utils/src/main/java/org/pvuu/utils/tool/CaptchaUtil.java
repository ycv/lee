...
public class CaptchaUtil {
     ...
     * 为一个验证码生成一个图片
     * <p>
     * 特性： - 颜色随机 - 上下位置随机 - 左右位置随机，但字符之间不会重叠 - 左右随机旋转一个角度 - 避免字符出界 - 随机颜色的小字符做背景干扰 - 根据字符大小自动调整图片大小、自动计算干扰字符的个数
     *
     * @param captcha
     * @return
     * @author XuJijun
    ...
    public static BufferedImage genCaptchaImg(String captcha) {
        ...
        // 验证码的颜色
        Color[] catchaColor =
            {Color.MAGENTA, Color.BLACK, Color.BLUE, Color.CYAN, Color.GREEN, Color.ORANGE, Color.PINK};
        ...
        for (int i = 0; i < count; i++) {
            ...
            // X、Y坐标在合适的范围内随机，不旋转：
            // g.drawString(String.valueOf(c), width/count*i+r.nextInt(width/count-fontSize),
